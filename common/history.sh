...
# the shell exits. It lives inside the say-hi tree ($_HI_HOME/say-hi) rather
# than $TMPDIR - a mktemp'd name there stays out of the ssh payload, which
# only ever tars the four enumerated $_HI_PAYLOAD members, so a session's
# scratch history is never relayed onward. common/bash.sh and common/zsh.zsh
# source this and point HISTFILE into it; fish can't source it and keeps its
# own copy in
...
if [ -z "${_HI_TMPDIR:-}" ]; then
  _HI_TMPDIR="$(mktemp -d "$_HI_HOME/say-hi/hi.history.XXXXXX")"
  export _HI_TMPDIR
