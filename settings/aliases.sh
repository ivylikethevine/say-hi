...
# stdin -> the client's clipboard (common/osc52.sh). The `[ -f ]` matters: the
# container fallback ships this file without paths.sh, where an empty
# $_HI_OSC52 would make `sh ` an alias that opens a shell.
...
# rebind (not bat/batcat/batn) is behind _HI_DISABLE_BAT_ALIAS.
...
# styled wrappers are behind _HI_DISABLE_LS_ALIASES; $_HI_EXA_BIN/$_HI_EZA_BIN
