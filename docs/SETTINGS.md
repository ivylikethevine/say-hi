...
| `_HI_CONTAINER_CLIS`         | `docker podman nerdctl finch`   | the docker-compatible CLIs, space-separated, that `hi <TAB>` lists running containers through and `hi <target>` resolves them with, each its own kind and `--use <arm>` target; a name that is not on `$PATH` costs nothing, and two that front one daemon list each container once. Add a drop-in or drop one you never use; names are plain identifiers, see [HI.51](GLOSSARY.md#hi51-docker-compatible-cli-family)                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
