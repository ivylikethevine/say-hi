...
| `_HI_DISABLE_HELIX`        | `0`                                                  | `hi --configure`          | turns off hi's helix config alone - the `hx` alias - where `_HI_DISABLE_EDITORS` turns off every editor's; `$EDITOR` can still pick it, bare |
