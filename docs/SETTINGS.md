...
from a hi shell: once the rc has run, every `_HI_*` name except seven is a
...
| `_HI_ASCII`                 | by locale                                            | `hi --configure` advanced | `1` forces ASCII stand-ins for the banner/prompt/packages glyphs and for the box rules `hi --preview` draws its tables with, `0` forces the glyphs; unset asks the locale, so `LANG=C` degrades cleanly instead of printing mojibake                                                                                                                                                                                        |
