...
| `minimal`    | on targets only the colored prompt: no header, git status, or editors — and nothing on this machine (`_HI_DISABLE_LOCAL=1`). |

A preset is an absolute answer over the feature toggles, the banner, and the
package check's groups: what it names is set and the rest of those return to
...
| `_HI_DISABLE_EDITORS`    | `0`                                                                              | `hi --configure`          | turns off the editor config overrides (vim, neovim, nano, emacs, micro, helix, kakoune) and, on a target, the `$EDITOR`/`$VISUAL`/`$SUDO_EDITOR` export that carries them into `git commit`, `crontab -e`, and `sudo -e`; the tmux, screen, and zellij config aliases follow their overlay files alone                                                                                                                                                                                                                                          |
...
| `_HI_MICRO_OPTS`         | on a target, `-backup false -savehistory false -mkparents true -diffgutter true` | you                       | the `micro` alias's flags - any micro setting works as `-name value`; with an overlay `micro/` the default is the first two alone, so its `settings.json` wins. At home with no overlay `micro/` there is no default and no alias: micro reads its own config directory                                                                                                                                                                                                                                                                         |
