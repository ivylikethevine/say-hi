...
| `copyright_per_file`                                                                | U _(partial)_                  | `# Copyright the say-hi contributors.` heads every shell source file except the shared CI helpers - the scripts of `.github/actions/`'s `apt`, `setup-tool`, and `setup-tools`, and five of `.github/scripts/` (`check_tool_versions.sh`, `find_tree_run.sh`, `release_notes.sh`, `release_slot.sh`, `scan_pinned_images.sh`), which carry only the SPDX line. |
