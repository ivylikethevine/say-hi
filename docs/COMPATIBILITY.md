...
| PowerShell                                       | ❌ not styled, by design                                              | a host with no `sh` still gets a session: a plain PowerShell one after hi's one-line notice (see [Shells hi does not style](#shells-hi-does-not-style)) |
...
| WinRM / PowerShell Remoting                                                                               | the answer [the OS table](#the-targets-os) gives stock Windows OpenSSH: hi's payload is POSIX shell, which PowerShell can neither source nor run the fallback ladder for. Windows with Git Bash on `PATH` is the supported shape                                                                                                                                                                                                                                                                                                                                                                                                                              |
