...
| ------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| `sed -E 's/(a\|b)/x/'`                | `sed -r`, or a backslash-bar alternation in a basic `sed` | BSD sed (macOS, FreeBSD, OpenBSD): `-r` is unknown and backslash-bar is a literal bar, so the match silently fails                     | `drift` (`sed -r`); the alternation by review |
...
| awk as POSIX writes it                | gawk's `gensub`, `strftime`, `\y`, arrays of arrays       | mawk, busybox awk, and onetrue (BSD) awk; hi.sh's include scan is checked against all four                                             | review                                        |
...
| strict mode on at the top of a sourced file, off at its end | leaving `set -euo pipefail` on in a file an interactive shell sources | the user's shell dies on the next non-zero status ([HI.15](GLOSSARY.md#hi15-strict-mode-bracketing))                                                                                              | review                                                                 |
...
| `printf -v x '%s' ''` to clear an out-var | `printf -v x ''`                    | bash 3.2 skips the assignment ([HI.05](GLOSSARY.md#hi05-printf--v-out-var))      | review                                         |
...
| `stat -c '%a' f 2>/dev/null \|\| stat -f '%Lp' f` | `stat -c` alone  | BSD and macOS `stat` spell formats with `-f`                                       | review    |
| `cd -P "$d" && pwd -P`                            | `readlink -f`    | absent from older macOS                                                            | `drift`   |
| `mktemp "${TMPDIR:-/tmp}/hi.name.XXXXXX"`         | `mktemp -t name` | `-t` means a template on GNU and a prefix on BSD                                   | review    |
...
| `_hi_shquote`                                               | `${2//\'/…}`                                        | 3.2 keeps the replacement's quoting ([HI.40](GLOSSARY.md#hi40-hand-rolled-sh-quoting))         | review                             |
...
| `sed -E` when a test extracts with alternation                           | a basic `sed` with backslash-bar alternation    | the macOS job's BSD sed                              | review                       |
