...
| `printf -v` into a plain variable, then `a[i]="$v"`         | `printf -v "a[i]"`, or `printf -v "$1"` handed `a[i]` | bash 3.2: "`a[i]`: not a valid identifier", and the element keeps its old value                |
